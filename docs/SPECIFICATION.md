...
* アンビエント流れ星（`playing` 中のみ表示。小さな点が一定速度で左へゆっくり流れ、画面外に出たら右端へリサイクルする軽量な演出。常時24個程度に抑え、パフォーマンスを優先）
* 流れ星（レベル3以上でランダム出現。アンビエント流れ星とは別の、レアな一直線の光跡演出）
...
* タイトルロゴ「NEON NEKO RUNNER」（シアン発光、サイン波による小さな上下の浮遊アニメーション付き）
* ゲーム内容（COLLECT FISH、AVOID OBSTACLES）とベストスコア・累計魚取得数・実績解除数を表示
* 現在選択中のゲームモード名（「MODE : CLASSIC」）・現在装備中のスキン名（「SKIN : CYBER CAT」、スキン自体のカラーで表示）を表示
* 操作説明を以下の形式で表示
  * `SPACE : START`（最も基本の操作のため、他の項目よりも目立つ色・サイズ・パルス演出で強調）
  * `M : MODE`
  * `K : SKINS`
  * `C : MISSIONS`
  * `S : SETTINGS`
...
* 一時停止中は「PAUSED」「現在のモード（MODE : CLASSIC）」「現在のスキン（SKIN : CYBER CAT、スキンのカラーで表示）」「PRESS P TO RESUME」を表示
* ヒットストップ・スコアポップ・ハイスコア発光・シェイク・リング・実績通知など、最後まで再生したい演出のタイマーは `playing` 判定より前で更新されるため、一時停止中でも自然に終了する
* HUD（左上のスコア等・右上のアイテム残り時間）は `playing` 中のみ表示し、一時停止中を含む他の画面では表示しない（後述「UI演出」参照）

## UI演出（Visual & UX Refresh）

* **HUD表示ルール**: 画面左上のスコア・レベル・モード等、画面右上のアイテム残り時間、コンボポップアップは `gameState === 'playing'` のときのみ表示する。タイトル・設定・スキン・モード選択・ミッション・実績・一時停止・ゲームオーバーの各画面では表示しない
* **選択中項目の強調**: キーボード操作中心のUIのため、設定画面の選択行・スキン選択画面のスキン名・モード選択画面のモード名・ミッション画面のミッション名には、サイン波による控えめな拡大（パルス）アニメーションを適用し、現在選択中の項目を一目で分かるようにする（共通ヘルパー `drawPulsingCenteredText`）
* **通知のスライドイン演出**: 実績解除・スキン解放・ミッション達成の通知（画面右上）は「右からスライドイン → 一定時間表示 → フェードアウト」の流れで表示する。スライドインは位置のみ、フェードアウトは透明度のみで表現し、既存の通知キュー構造（`achievementQueue` / `skinUnlockQueue` / `missionQueue` とそれぞれの `current*Notice`）はそのまま維持する（共通ヘルパー `getNoticeAnimation`）
* **タイトルの浮上アニメーション**: タイトルロゴはサイン波で上下に小さく揺れる（演出は控えめにとどめる）
* **スコアポップ**: スコア更新時に数字が少し拡大して元のサイズへ戻るアニメーション（Version 1.0時点から実装済み、本バージョンでの変更なし）
* いずれのアニメーションも `requestAnimationFrame` による既存のメインループ（`update()` → `render()`）内で、`performance.now()` を用いた軽量な計算のみで実現し、新たなタイマー管理の仕組みは追加しない
